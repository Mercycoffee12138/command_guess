...
void MD5Hash_SIMD(const string inputs[4], bit32 states[4][4]) {
    ...
    for (int i = 1; i < 4; i++) {
        assert(messageLengths[i] / 64 == n_blocks);
    }
    
    // 处理每个块
    for (size_t block = 0; block < n_blocks; block++) {
        // 从每个消息中提取当前块的数据，注意字节序
        bit32x4_t M[16];
        for (int j = 0; j < 16; j++) {
            bit32 values[4];
            for (int i = 0; i < 4; i++) {
                ...
                // 使用相同的字节序处理
                values[i] = (bit32)block_ptr[0] | 
                           ((bit32)block_ptr[1] << 8) | 
                           ((bit32)block_ptr[2] << 16) | 
                           ((bit32)block_ptr[3] << 24);
            }
    ...
    // 保存结果
    for (int i = 0; i < 4; i++) {
        states[i][0] = a_values[i];
        states[i][1] = b_values[i];
        states[i][2] = c_values[i];
        states[i][3] = d_values[i];
    }
